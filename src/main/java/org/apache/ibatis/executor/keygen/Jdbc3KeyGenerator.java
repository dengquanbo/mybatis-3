...
/**
 * 实现 KeyGenerator 接口，基于 {@link Statement#getGeneratedKeys} 方法的 KeyGenerator 实现类，适用于 MySQL、H2 主键生成
 *
        ...
        if (firstDot == -1) {
            throw new ExecutorException(
                    "Could not determine which parameter to assign generated keys to. "
                            + "Note that when there are multiple parameters, 'keyProperty' must include the parameter" +
                            " name (e.g. 'param.id'). "
    ...
    private TypeHandler<?>[] getTypeHandlers(TypeHandlerRegistry typeHandlerRegistry, MetaObject metaParam,
        ...
        for (int i = 0; i < keyProperties.length; i++) {
            if (metaParam.hasSetter(keyProperties[i])) {
                ...
                typeHandlers[i] = typeHandlerRegistry.getTypeHandler(keyPropertyType,
